...
public interface LicenciaTenantRepository extends JpaRepository<LicenciaTenant, Long> {
    ...
    @Query("SELECT l FROM LicenciaTenant l WHERE l.activa = true AND l.fechaVencimientoPago < :fecha")
    List<LicenciaTenant> buscarVencidasActivas(@Param("fecha") LocalDate fecha);
}
