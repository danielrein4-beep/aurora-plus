...
function portada(doc: jsPDF, datos: DatosPdfInformeVertical, color: RGB, fecha: string) {
  ...
  tarjetas(doc, y0, [
    ...
    { label: "INGRESOS DEL PERÍODO", val: usd(Number(k.ingresosPeriodo)), color: CIELO, nota: `${usd(Number(k.ingresosHistorico))} históricos` },
    { label: "USUARIOS", val: numero.format(k.usuarios), color: VIOLETA, nota: `${k.altasPeriodo} negocio(s) nuevo(s)` },
  ]);
...
function hallazgos(datos: DatosPdfInformeVertical): { texto: string; color: RGB }[] {
  ...
  salida.push({ texto: `Adopción: ${k.conActividad} de ${k.activos} negocios activos (${adopcion}%) registraron operaciones en el período.`, color: adopcion >= 60 ? ESMERALDA : adopcion >= 30 ? AMBAR : ROJO });
  ...
  if (m?.serie) {
    ...
    if (previo > 0) {
      ...
      salida.push({ texto: `${m.etiqueta} en ${nombreMes(d.meses[10])} (último mes completo): ${v >= 0 ? "+" : ""}${v}% frente a ${nombreMes(d.meses[9])}.`, color: v >= 0 ? ESMERALDA : AMBAR });
    }
  }
  ...
  if (top && top.uso > 0) salida.push({ texto: `El negocio más activo es ${top.n.nombre}, con ${numero.format(top.uso)} registros en el período.`, color: VIOLETA });

  salida.push({ texto: `Ingresos de suscripciones: ${usd(Number(k.ingresosPeriodo))} en el período y ${usd(Number(k.ingresosHistorico))} en total.`, color: CIELO });
  return salida;
}
...
function metricasYEvolucion(doc: jsPDF, datos: DatosPdfInformeVertical, color: RGB, fecha: string) {
  ...
  y = tabla(doc, [
    ...
    return {
      ...
      negrita: [0, 1],
      colores: (v == null ? {} : { 7: v >= 0 ? ESMERALDA : ROJO }) as Record<number, RGB>,
    };
...
function produccionGanaderia(doc: jsPDF, datos: DatosPdfInformeVertical, op: ProduccionGanaderia, color: RGB, fecha: string) {
  ...
  tarjetas(doc, y0, [
    ...
    { label: "ANIMALES EN HATO", val: numero.format(hato), color: ESMERALDA, nota: "todas las fincas" },
  ]);
