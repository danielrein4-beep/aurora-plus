import React, { useState } from 'react';
import { PeriodoNomina, ReciboNominaEmpleado } from './types';
import { EstadoNominaBadge } from './EstadoNominaBadge';
import { DetalleCalculoNomina } from './DetalleCalculoNomina';

interface NominaPersonalProps {
  periodos: PeriodoNomina[];
  ocultarSueldo: boolean;
}

export const NominaPersonal: React.FC<NominaPersonalProps> = ({
  periodos: initialPeriodos,
  ocultarSueldo,
}) => {
  const [periodos, setPeriodos] = useState<PeriodoNomina[]>(initialPeriodos);
  const [periodoActivo, setPeriodoActivo] = useState<PeriodoNomina>(initialPeriodos[0]);
  const [reciboDetalle, setReciboDetalle] = useState<ReciboNominaEmpleado | null>(null);
  const [modalRevisionAbierto, setModalRevisionAbierto] = useState(false);
  const [modalAjusteAbierto, setModalAjusteAbierto] = useState(false);
  const [motivoAjuste, setMotivoAjuste] = useState('');
  const [toastMensaje, setToastMensaje] = useState<string | null>(null);
  const [moduloHabilitado, setModuloHabilitado] = useState(true);

  const mostrarNotificacion = (msg: string) => {
    setToastMensaje(msg);
    setTimeout(() => setToastMensaje(null), 4000);
  };

  const handleAprobarPeriodo = () => {
    const actualizado: PeriodoNomina = {
      ...periodoActivo,
      estado: 'APROBADA',
      aprobadoPor: 'Dirección Médica y Finanzas [DEMO]',
      fechaAprobacion: new Date().toLocaleString(),
      historialAjustes: [
        {
          id: `AJU-${Date.now()}`,
          fecha: new Date().toLocaleString(),
          tipoAccion: 'APROBACION_INICIAL',
          autor: 'Usuario Administrativo [DEMO]',
          motivoJustificado: 'Aprobación definitiva del período tras revisión previa de auditoría.',
        },
        ...periodoActivo.historialAjustes,
      ],
    };

    setPeriodos(periodos.map((p) => (p.id === actualizado.id ? actualizado : p)));
    setPeriodoActivo(actualizado);
    setModalRevisionAbierto(false);
    mostrarNotificacion(`Período ${actualizado.codigoPeriodo} aprobado exitosamente. Estado inmutable activado.`);
  };

  const handleRegistrarAjuste = (tipo: 'AJUSTE_POSTERIOR' | 'REVERSO_TOTAL') => {
    if (!motivoAjuste.trim()) {
      alert('Debe indicar una justificación de auditoría válida.');
      return;
    }

    const nuevoEstado = tipo === 'REVERSO_TOTAL' ? 'REVERSADA' : 'AJUSTADA';
    const actualizado: PeriodoNomina = {
      ...periodoActivo,
      estado: nuevoEstado,
      historialAjustes: [
        {
          id: `AJU-${Date.now()}`,
          fecha: new Date().toLocaleString(),
          tipoAccion: tipo,
          autor: 'Auditor Financiero [DEMO]',
          motivoJustificado: motivoAjuste,
        },
        ...periodoActivo.historialAjustes,
      ],
    };

    setPeriodos(periodos.map((p) => (p.id === actualizado.id ? actualizado : p)));
    setPeriodoActivo(actualizado);
    setModalAjusteAbierto(false);
    setMotivoAjuste('');
    mostrarNotificacion(`Acción de ${tipo === 'REVERSO_TOTAL' ? 'reverso' : 'ajuste'} registrada con respaldo de auditoría.`);
  };

  return (
    <div className="space-y-6">
      {/* Toast Notification */}
      {toastMensaje && (
        <div className="p-3 bg-[#10b981]/20 border border-[#10b981]/40 rounded-xl text-xs text-[#34d399] font-medium flex items-center justify-between animate-fade-in">
          <span>{toastMensaje}</span>
          <button onClick={() => setToastMensaje(null)} className="text-xs text-[#34d399] hover:underline">
            ✕
          </button>
        </div>
      )}

      {/* Banner Prominente: Módulo Opcional y Desactivado por Defecto */}
      <div className="p-4 bg-[#131c2e] border-2 border-[#1e2d48] rounded-2xl space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="px-2 py-0.5 rounded bg-[#f59e0b]/15 text-[#fbbf24] font-mono text-[11px] font-bold border border-[#f59e0b]/30">
                MÓDULO OPCIONAL
              </span>
              <span className="px-2 py-0.5 rounded bg-[#1e293b] text-[#94a3b8] font-mono text-[11px]">
                Desactivado por Defecto
              </span>
              <span className="text-[11px] font-mono text-[#35d7c3]">[DEMO]</span>
            </div>
            <h3 className="text-base font-bold text-[#f8fafc]">
              Aurora Nómina: Liquidación y Control Interno
            </h3>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-xs text-[#94a3b8]">Estado del módulo:</span>
            <button
              onClick={() => {
                setModuloHabilitado(!moduloHabilitado);
                mostrarNotificacion(`Módulo de nómina ${!moduloHabilitado ? 'habilitado' : 'pausado'} en este espacio [DEMO]`);
              }}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${
                moduloHabilitado
                  ? 'bg-[#35d7c3] text-black hover:bg-[#28b8a6]'
                  : 'bg-[#1e293b] text-[#94a3b8] hover:text-[#f8fafc]'
              }`}
            >
              {moduloHabilitado ? '✓ Habilitado' : 'Pausado'}
            </button>
          </div>
        </div>

        <p className="text-xs text-[#94a3b8] leading-relaxed border-t border-[#1e293b] pt-2">
          <strong className="text-[#cbd5e1]">Aviso de Cumplimiento:</strong> Este módulo es un sistema de cálculo interno referencial y control gerencial de percepciones y deducciones. No sustituye ni simula sistemas tributarios oficiales ni ejecuta pasarelas directas con entidades gubernamentales.
        </p>
      </div>

      {/* Selector de Períodos y Resumen Financiero */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Lista de Períodos */}
        <div className="p-4 bg-[#131c2e] border border-[#1e2d48] rounded-xl space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="font-semibold text-xs text-[#94a3b8] uppercase tracking-wider">
              Períodos de Nómina
            </h4>
            <span className="font-mono text-xs text-[#35d7c3]">{periodos.length} Ciclos</span>
          </div>

          <div className="space-y-2">
            {periodos.map((p) => {
              const esSeleccionado = p.id === periodoActivo.id;
              return (
                <div
                  key={p.id}
                  onClick={() => setPeriodoActivo(p)}
                  className={`p-3 rounded-xl border cursor-pointer transition-all ${
                    esSeleccionado
                      ? 'bg-[#1a2438] border-[#35d7c3] text-[#f8fafc]'
                      : 'bg-[#0f172a] border-[#1e293b] text-[#cbd5e1] hover:border-[#35d7c3]/40'
                  }`}
                >
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <span className="font-mono text-xs font-semibold text-[#35d7c3]">
                      {p.codigoPeriodo}
                    </span>
                    <EstadoNominaBadge estado={p.estado} />
                  </div>
                  <h5 className="font-medium text-xs text-[#f8fafc]">{p.nombre}</h5>
                  <div className="flex items-center justify-between text-[11px] text-[#94a3b8] mt-2 font-mono">
                    <span>{p.totalEmpleados} empleados</span>
                    <span className="text-[#35d7c3] font-bold">
                      {ocultarSueldo ? '••••••' : `$${p.montoTotalNeto.toFixed(2)} USD`}
                    </span>
                  </div>
                </div>
              );
            })}
          </div>
        </div>

        {/* Panel del Período Seleccionado */}
        <div className="lg:col-span-2 p-5 bg-[#131c2e] border border-[#1e2d48] rounded-xl space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#1e2d48]">
            <div>
              <div className="flex items-center gap-2">
                <h4 className="font-bold text-base text-[#f8fafc]">{periodoActivo.nombre}</h4>
                <EstadoNominaBadge estado={periodoActivo.estado} />
              </div>
              <p className="text-xs text-[#94a3b8] font-mono mt-0.5">
                Vigencia: {periodoActivo.fechaInicio} al {periodoActivo.fechaFin} | Pago estimado: {periodoActivo.fechaTentativaPago}
              </p>
            </div>

            {/* Controles de Estado de Nómina */}
            <div className="flex items-center gap-2">
              {periodoActivo.estado === 'BORRADOR' || periodoActivo.estado === 'EN_REVISION' ? (
                <button
                  onClick={() => setModalRevisionAbierto(true)}
                  className="px-3.5 py-2 rounded-lg bg-[#35d7c3] hover:bg-[#28b8a6] text-black font-semibold text-xs transition-colors focus:outline-none focus:ring-2 focus:ring-white"
                >
                  🔍 Revisar y Aprobar Período
                </button>
              ) : (
                <div className="flex items-center gap-2">
                  <span className="text-[11px] text-[#34d399] font-mono bg-[#10b981]/15 px-2 py-1 rounded border border-[#10b981]/30">
                    🔒 Nómina Bloqueada
                  </span>
                  <button
                    onClick={() => setModalAjusteAbierto(true)}
                    className="px-3 py-1.5 rounded-lg bg-[#1e293b] hover:bg-[#334155] text-xs font-medium text-[#cbd5e1] border border-[#334155]"
                  >
                    Ajustar / Reversar
                  </button>
                </div>
              )}
            </div>
          </div>

          {/* Totales de Liquidación */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="p-3 bg-[#0f172a] rounded-xl border border-[#1e293b]">
              <span className="text-xs text-[#64748b] block">Total Percepciones Brutas</span>
              <span className="font-mono text-base font-bold text-[#f8fafc]">
                {ocultarSueldo ? '••••••' : `$${periodoActivo.montoTotalBruto.toFixed(2)} USD`}
              </span>
            </div>
            <div className="p-3 bg-[#0f172a] rounded-xl border border-[#1e293b]">
              <span className="text-xs text-[#64748b] block">Total Deducciones</span>
              <span className="font-mono text-base font-bold text-[#f87171]">
                {ocultarSueldo ? '••••••' : `-$${periodoActivo.montoTotalDeducciones.toFixed(2)} USD`}
              </span>
            </div>
            <div className="p-3 bg-[#0f172a] rounded-xl border border-[#1e293b]">
              <span className="text-xs text-[#64748b] block">Monto Neto a Desembolsar</span>
              <span className="font-mono text-base font-bold text-[#35d7c3]">
                {ocultarSueldo ? '••••••' : `$${periodoActivo.montoTotalNeto.toFixed(2)} USD`}
              </span>
            </div>
          </div>

          {/* Recibos Desglosados por Empleado */}
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h5 className="font-semibold text-xs uppercase tracking-wider text-[#94a3b8]">
                Recibos de Pago Individuales ({periodoActivo.recibos.length})
              </h5>
              <span className="text-xs text-[#64748b]">Clic para inspeccionar fórmula</span>
            </div>

            <div className="space-y-2">
              {periodoActivo.recibos.map((recibo) => (
                <div
                  key={recibo.id}
                  onClick={() => setReciboDetalle(recibo)}
                  className="p-3 bg-[#0f172a] border border-[#1e293b] rounded-xl flex items-center justify-between gap-3 text-xs cursor-pointer hover:border-[#35d7c3]/50 transition-colors"
                >
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-[#f8fafc] text-sm">{recibo.empleadoNombre}</span>
                      <span className="text-[10px] px-1.5 py-0.2 rounded bg-[#1e293b] text-[#94a3b8]">
                        {recibo.departamento}
                      </span>
                    </div>
                    <p className="text-[#64748b] text-xs">{recibo.empleadoCargo}</p>
                  </div>

                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <span className="font-mono font-bold text-[#35d7c3] text-sm block">
                        {ocultarSueldo ? '••••••' : `$${recibo.montoNetoPagar.toFixed(2)} ${recibo.moneda}`}
                      </span>
                      <span className="text-[10px] text-[#94a3b8]">
                        {recibo.conceptosDesglosados.length} conceptos
                      </span>
                    </div>
                    <span className="text-[#35d7c3]">🔍</span>
                  </div>
                </div>
              ))}
            </div>
          </div>

          {/* Historial de Auditoría / Ajustes */}
          {periodoActivo.historialAjustes.length > 0 && (
            <div className="p-4 bg-[#0b111e] rounded-xl border border-[#1e293b] space-y-2 text-xs">
              <h5 className="font-semibold text-[#94a3b8] uppercase tracking-wider text-[11px]">
                Bitácora de Auditoría del Período
              </h5>
              <div className="divide-y divide-[#1e293b]">
                {periodoActivo.historialAjustes.map((h) => (
                  <div key={h.id} className="py-2 space-y-0.5">
                    <div className="flex items-center justify-between font-mono text-[11px]">
                      <span className="text-[#35d7c3] font-semibold">{h.tipoAccion}</span>
                      <span className="text-[#64748b]">{h.fecha}</span>
                    </div>
                    <p className="text-[#cbd5e1]">{h.motivoJustificado}</p>
                    <span className="text-[10px] text-[#94a3b8]">Registrado por: {h.autor}</span>
                  </div>
                ))}
              </div>
            </div>
          )}
        </div>
      </div>

      {/* Modal de Detalle de Fórmulas y Cálculos de un Recibo */}
      {reciboDetalle && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#131c2e] border border-[#1e2d48] w-full max-w-2xl rounded-2xl p-6 space-y-4 shadow-2xl max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-[#1e2d48]">
              <div>
                <h3 className="text-base font-bold text-[#f8fafc]">
                  Inspección Analítica de Recibo [DEMO]
                </h3>
                <p className="text-xs text-[#94a3b8]">Desglose de bases de cálculo y reglas aplicadas</p>
              </div>
              <button
                onClick={() => setReciboDetalle(null)}
                className="text-[#94a3b8] hover:text-[#f8fafc]"
              >
                ✕
              </button>
            </div>

            <DetalleCalculoNomina recibo={reciboDetalle} ocultarSueldo={ocultarSueldo} />

            <div className="flex justify-end pt-3 border-t border-[#1e2d48]">
              <button
                onClick={() => setReciboDetalle(null)}
                className="px-4 py-2 rounded-lg bg-[#1e293b] hover:bg-[#334155] text-xs text-[#f8fafc]"
              >
                Cerrar Desglose
              </button>
            </div>
          </div>
        </div>
      )}

      {/* Modal de Pre-Revisión antes de Aprobación */}
      {modalRevisionAbierto && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#131c2e] border border-[#1e2d48] w-full max-w-xl rounded-2xl p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between pb-3 border-b border-[#1e2d48]">
              <div>
                <h3 className="text-base font-bold text-[#f8fafc]">
                  Pantalla de Revisión Previa de Nómina
                </h3>
                <span className="text-xs text-[#fbbf24] font-medium">
                  Validación obligatoria antes de congelar período
                </span>
              </div>
              <button
                onClick={() => setModalRevisionAbierto(false)}
                className="text-[#94a3b8] hover:text-[#f8fafc]"
              >
                ✕
              </button>
            </div>

            <div className="space-y-3 text-xs text-[#cbd5e1]">
              <p className="leading-relaxed">
                Está a punto de aprobar el período{' '}
                <strong className="text-[#f8fafc]">{periodoActivo.nombre}</strong>. Una vez aprobado, el período pasará a estado inmutable y no admitirá modificaciones directas.
              </p>

              <div className="p-4 bg-[#0f172a] rounded-xl border border-[#1e293b] space-y-2 font-mono">
                <div className="flex justify-between">
                  <span className="text-[#94a3b8]">Colaboradores a liquidar:</span>
                  <span className="text-[#f8fafc] font-bold">{periodoActivo.recibos.length}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#94a3b8]">Total Bruto:</span>
                  <span className="text-[#f8fafc]">${periodoActivo.montoTotalBruto.toFixed(2)} USD</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#94a3b8]">Total Deducciones:</span>
                  <span className="text-[#f87171]">-${periodoActivo.montoTotalDeducciones.toFixed(2)} USD</span>
                </div>
                <div className="flex justify-between pt-2 border-t border-[#1e293b] text-sm">
                  <span className="text-[#35d7c3] font-bold">Total Neto a Pagar:</span>
                  <span className="text-[#35d7c3] font-bold">${periodoActivo.montoTotalNeto.toFixed(2)} USD</span>
                </div>
              </div>

              <div className="p-3 bg-[#f59e0b]/10 border border-[#f59e0b]/25 rounded-lg text-[#fbbf24] text-[11px]">
                ⚠️ Recordatorio: Los cambios posteriores deberán realizarse exclusivamente mediante notas de ajuste o reverso justificado.
              </div>
            </div>

            <div className="flex justify-end gap-2 pt-3 border-t border-[#1e2d48]">
              <button
                onClick={() => setModalRevisionAbierto(false)}
                className="px-4 py-2 rounded-lg bg-[#1e293b] text-xs text-[#cbd5e1] hover:bg-[#334155]"
              >
                Cancelar
              </button>
              <button
                onClick={handleAprobarPeriodo}
                className="px-4 py-2 rounded-lg bg-[#35d7c3] hover:bg-[#28b8a6] text-black font-semibold text-xs"
              >
                Confirmar y Congelar Nómina
              </button>
            </div>
          </div>
        </div>
      )}

      {/* Modal de Registro de Ajuste o Reverso */}
      {modalAjusteAbierto && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#131c2e] border border-[#1e2d48] w-full max-w-md rounded-2xl p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between pb-2 border-b border-[#1e2d48]">
              <h3 className="text-base font-bold text-[#f8fafc]">
                Ajuste / Reverso de Período [DEMO]
              </h3>
              <button
                onClick={() => setModalAjusteAbierto(false)}
                className="text-[#94a3b8] hover:text-[#f8fafc]"
              >
                ✕
              </button>
            </div>

            <div className="space-y-3 text-xs">
              <p className="text-[#94a3b8]">
                Indique el motivo justificado que quedará asentado en la bitácora inmutable de auditoría:
              </p>

              <div>
                <label className="text-[#94a3b8] block mb-1">Motivo de la Corrección:</label>
                <textarea
                  rows={3}
                  value={motivoAjuste}
                  onChange={(e) => setMotivoAjuste(e.target.value)}
                  placeholder="Ej. Corrección por reporte tardío de guardia de fin de semana..."
                  className="w-full bg-[#0b111e] border border-[#1e293b] rounded-lg p-2 text-[#f8fafc] text-xs"
                />
              </div>
            </div>

            <div className="flex flex-col sm:flex-row justify-end gap-2 pt-2 border-t border-[#1e2d48]">
              <button
                onClick={() => handleRegistrarAjuste('AJUSTE_POSTERIOR')}
                className="px-3.5 py-2 rounded-lg bg-[#6366f1] hover:bg-[#4f46e5] text-white font-medium text-xs"
              >
                Registrar Nota de Ajuste
              </button>
              <button
                onClick={() => handleRegistrarAjuste('REVERSO_TOTAL')}
                className="px-3.5 py-2 rounded-lg bg-[#ef4444] hover:bg-[#dc2626] text-white font-medium text-xs"
              >
                Reversar Período Completo
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
};
