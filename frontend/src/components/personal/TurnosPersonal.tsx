    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-xs sm:text-sm font-semibold text-slate-900 uppercase tracking-wider text-slate-500">
              Catálogo de Turnos y Jornadas Operativas
            </h3>
            <p className="text-xs text-slate-500">Esquemas horarios adaptados a salud, gastronomía y campo</p>
          </div>
          <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-slate-50 text-[#177E89] border border-slate-200">
            PLANTILLAS
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {turnosHorarios.map((th) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span className="font-semibold text-slate-900">{th.nombre}</span>
                <span className="font-mono text-[11px] px-2 py-0.5 rounded bg-white text-[#177E89]">
              ...
              {th.descripcion && <p className="text-slate-500 text-[11px]">{th.descripcion}</p>}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            <label htmlFor="fecha-turnos" className="text-xs text-slate-500 font-medium">
              Fecha:
            </label>
            <input
              id="fecha-turnos"
          ...
          <div className="flex items-center gap-2">
            <label htmlFor="depto-turnos" className="text-xs text-slate-500 font-medium">
              Área:
            </label>
            <select
              id="depto-turnos"
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl overflow-hidden">
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between text-xs">
          ...
          <span className="text-slate-500 font-mono">{asignacionesFiltradas.length} Asignaciones</span>
        </div>
            ...
            {asignacionesFiltradas.map((asg) => (
              ...
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-slate-900 text-sm">{asg.empleadoNombre}</span>
                    <span className="text-[10px] px-1.5 py-0.2 rounded bg-white text-slate-500 font-mono">
                      {asg.departamento}
                    </span>
                  </div>
                  <p className="text-xs text-slate-500">{asg.empleadoCargo}</p>
                </div>

                <div className="flex items-center gap-4">
                  <div className="text-left sm:text-right">
                    <span className="font-medium text-sky-600 block">{asg.turnoNombre}</span>
                    <span className="text-[11px] text-slate-500 font-mono">Estado: {asg.estado}</span>
                  </div>
      ...
      {modalAsignarAbierto && (
        ...
        >
          <div className="bg-slate-50 border border-slate-200 w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl p-4 sm:p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between">
              <h3 id="modal-asignar-turno-titulo" className="text-base font-bold text-slate-900">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="text-slate-500 block mb-1">Colaborador:</label>
                <select
              ...
              <div>
                <label className="text-slate-500 block mb-1">Turno a Programar:</label>
                <select
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                onClick={() => setModalAsignarAbierto(false)}
                className="px-4 py-2 rounded-lg bg-slate-100 text-xs text-slate-700 hover:bg-slate-200"
              >
                Cancelar
              </button>
              <button
