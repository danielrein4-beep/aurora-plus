...
export const MetasPersonal: React.FC<MetasPersonalProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-[#FFFFFF] border border-[#E5E5EA] rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 text-xs">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="px-2 py-0.2 rounded bg-[#177E89]/15 text-[#177E89] text-[10px]">
        ...
        <div className="flex items-center gap-2 text-[11px] text-[#1D1D1F] bg-[#F5F5F7] px-3 py-2 rounded-lg border border-[#E5E5EA] whitespace-nowrap">
          <span>{metas.filter((m) => m.estado === 'COMPLETADA').length} de {metas.length} logradas</span>
        </div>
      </div>

      {/* Barra de Filtros */}
      <div className="p-4 bg-[#FFFFFF] border border-[#E5E5EA] rounded-xl flex flex-wrap gap-3 items-center justify-between">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2">
            <label htmlFor="tipo-meta" className="text-xs text-[#86868B] font-medium">
              Tipo de Meta:
            </label>
            <select
              id="tipo-meta"
          ...
          <div className="flex items-center gap-2">
            <label htmlFor="depto-meta" className="text-xs text-[#86868B] font-medium">
              Departamento:
            </label>
            <select
              id="depto-meta"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtradas.map((meta) => {
          ...
          return (
            <div
              key={meta.id}
              className="p-5 bg-[#FFFFFF] border border-[#E5E5EA] rounded-xl space-y-4 hover:border-[#177E89]/40 transition-colors flex flex-col justify-between"
            >
              <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${
                          esAutomatica
                            ? 'bg-[#177E89]/15 text-[#177E89] border-[#177E89]/30'
                            : 'bg-[#F5F5F7] text-[#1D1D1F] border-[#E5E5EA]'
                        }`}
                      >
                        {esAutomatica ? 'Automática (ERP)' : 'Manual (Supervisión)'}
                      </span>
                      <span className="text-[10px] px-2 py-0.5 rounded bg-[#F5F5F7] text-[#86868B]">
                        {meta.departamentoObjetivo}
                      </span>
                    </div>
                    <h4 className="font-semibold text-[#1D1D1F] text-sm mt-1.5">{meta.titulo}</h4>
                  </div>
                ...
                <p className="text-xs text-[#86868B] leading-relaxed">{meta.descripcion}</p>
              </div>

              <div className="space-y-2 pt-2 border-t border-[#E5E5EA]">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-[#86868B]">Progreso computado:</span>
                ...
                <div className="w-full bg-[#F5F5F7] h-2.5 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-300 ${
                      meta.estado === 'COMPLETADA' ? 'bg-[#177E89]' : 'bg-[#177E89]'
                    }`}
                ...
                {meta.reconocimiento && (
                  <div className="p-2 bg-[#F5F5F7] rounded-lg border border-[#E5E5EA] text-[11px] text-[#1D1D1F] flex items-center gap-1.5">
                    <span className="text-[#177E89]">OK</span>
        ...
        >
          <div className="bg-[#FFFFFF] border border-[#E5E5EA] w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl p-4 sm:p-6 space-y-4 shadow-sm">
            <div className="flex items-center justify-between">
              <h3 id="modal-crear-meta-titulo" className="text-base font-bold text-[#1D1D1F]">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="text-[#86868B] block mb-1">Título del Objetivo:</label>
                <input
                  type="text"
                  value={nuevoTitulo}
                  onChange={(e) => setNuevoTitulo(e.target.value)}
                  placeholder="Ej. Tiempo de atención en mesa, Control de inventario..."
                  className="w-full bg-[#F5F5F7] border border-[#E5E5EA] rounded-lg p-2 text-[#1D1D1F] focus:outline-none focus:ring-2 focus:ring-[#177E89]"
                />
              </div>

              <div>
                <label className="text-[#86868B] block mb-1">Descripción:</label>
                <textarea
                  rows={2}
              ...
              <div>
                <label htmlFor="meta-empleado" className="text-[#86868B] block mb-1">Empleado:</label>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label htmlFor="meta-desde" className="text-[#86868B] block mb-1">Desde:</label>
                  <input id="meta-desde" type="date" value={fechaInicio} onChange={(e) => setFechaInicio(e.target.value)}
                    className="w-full bg-[#F5F5F7] border border-[#E5E5EA] rounded-lg p-2 text-[#1D1D1F] focus:outline-none focus:ring-2 focus:ring-[#177E89]"
                  />
                </div>
                <div>
                  <label htmlFor="meta-hasta" className="text-[#86868B] block mb-1">Hasta:</label>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[#86868B] block mb-1">Meta a Alcanzar:</label>
                  <input
                    type="number"
                    value={nuevoValor}
                ...
                <div>
                  <label className="text-[#86868B] block mb-1">Unidad:</label>
                  <input
                    type="text"
                    value={nuevaUnidad}
