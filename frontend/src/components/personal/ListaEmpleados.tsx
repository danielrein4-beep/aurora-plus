...
export const ListaEmpleados: React.FC<ListaEmpleadosProps> = ({
  ...
  const getEstadoBadge = (estado: EstadoEmpleado) => {
    switch (estado) {
      case 'ACTIVO':
        return (
          <span className="inline-flex items-center gap-1 text-[11px] font-normal text-[#177E89] bg-[#177E89]/10 px-2 py-0.5 rounded-full border border-[#177E89]/20">
      ...
      case 'DE_VACACIONES':
        return (
          <span className="inline-flex items-center gap-1 text-[11px] font-normal text-amber-600 bg-[#fbbf24]/10 px-2 py-0.5 rounded-full border border-[#fbbf24]/20">
      ...
      case 'LICENCIA':
        return (
          <span className="inline-flex items-center gap-1 text-[11px] font-normal text-sky-600 bg-[#38bdf8]/10 px-2 py-0.5 rounded-full border border-[#38bdf8]/20">
      ...
      case 'INACTIVO':
        return (
          <span className="inline-flex items-center gap-1 text-[11px] font-normal text-rose-600 bg-[#f87171]/10 px-2 py-0.5 rounded-full border border-[#f87171]/20">
      ...
      <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className={`px-3 py-2 rounded-lg text-xs font-normal border flex items-center gap-1.5 transition-colors focus:outline-none focus:ring-2 focus:ring-[#177E89] ${
              ocultarSueldo
        ...
        {filtrados.map((emp) => (
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-mono text-[11px] text-[#177E89] font-normal">{emp.codigoEmpleado}</span>
                  {getEstadoBadge(emp.estado)}
                </div>
                <h4 className="font-bold tracking-tight text-slate-900 text-sm mt-0.5">
                  {emp.nombre} {emp.apellidos}
                </h4>
                <p className="text-xs font-normal text-slate-500">{emp.cargo}</p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-slate-200">
              <div>
                <span className="text-slate-500 block text-[10px] font-normal">{v.area}</span>
                <span className="text-slate-700 truncate block font-normal">{v.nombreArea(emp.departamento)}</span>
              </div>
              <div>
                <span className="text-slate-500 block text-[10px] font-normal">Sueldo pactado</span>
            ...
            <div className="flex items-center justify-between text-xs font-normal text-slate-500 pt-1">
              <span>Turno: {emp.turnoAsignado}</span>
              <span className="text-[#177E89] font-normal">Ver Ficha &rarr;</span>
            </div>
          </div>
        ))}
        {filtrados.length === 0 && (
          <div className="p-8 text-center bg-slate-50 border border-slate-200 rounded-xl text-xs font-normal text-slate-500">
              ...
              {filtrados.map((emp) => (
                ...
                >
                  <td className="py-3 px-4">
                    <div className="font-mono text-[#177E89] font-normal">{emp.codigoEmpleado}</div>
                  ...
                  <td className="py-3 px-4">
                    ...
                    <div className="text-[11px] text-slate-500 font-normal">{emp.cargo}</div>
                  </td>
                  <td className="py-3 px-4 text-slate-500 font-normal">{emp.turnoAsignado}</td>
                  <td className="py-3 px-4">{getEstadoBadge(emp.estado)}</td>
                  <td className="py-3 px-4 text-right font-mono font-normal text-[#177E89]">
                    {ocultarSueldo ? '••••••' : formatearMoneda(emp.salarioBaseReferencial, emp.moneda)}
                  </td>
                  <td className="py-3 px-4 text-center">
                    <button
                      ...
                      className="px-2.5 py-1 rounded bg-white border border-slate-200 text-xs font-normal text-[#177E89] hover:border-[#177E89]/50 focus:outline-none focus:ring-2 focus:ring-[#177E89]"
                    >
                      Ver Ficha
                    </button>
                  </td>
