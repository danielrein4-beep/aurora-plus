...
import {
  CAJA, INPUT, BOTON, BOTON_ORO, BOTON_SUAVE, CATEGORIAS, ESTADOS_VE, dinero, numero, precioTexto, edadTexto, sexoTexto,
...
export default function FichaAnimal({ id, compradorInicial, puedeNegociar, conCondiciones, onVolver, onCambio }: {
  ...
  const compradores = useMemo(() => {
    ...
    (d?.ofertas ?? []).forEach((o) => { if (o.compradorTenantId && o.comprador) m.set(o.compradorTenantId, o.comprador.nombre); });
    return m;
  }, [d]);
  useEffect(() => {
  ...
  const totalEstimado = d.tipoPrecio === "POR_KG" && d.peso != null ? d.precio * d.peso : null;
    ...
    <div className="space-y-6">
      ...
      {aviso && (
        <div className={`px-4 py-3 rounded-2xl text-sm font-bold ${
          aviso.tipo === "error" ? "bg-rose-50 text-rose-700" : aviso.tipo === "info" ? "bg-amber-50 text-amber-900" : "bg-emerald-50 text-emerald-800"
        }`}>
          {aviso.texto}
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 space-y-3">
          ...
          {d.fotos.length > 1 && (
            <div className="flex gap-2 overflow-x-auto pb-1">
              {d.fotos.map((f, i) => (
                <button key={f.id} onClick={() => setFoto(i)} className={`w-24 h-18 aspect-[4/3] rounded-xl overflow-hidden shrink-0 border-2 cursor-pointer ${i === foto ? "border-amber-500" : "border-transparent opacity-70 hover:opacity-100"}`}>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className={`${CAJA} p-6 space-y-4`}>
            <div>
              <span className="inline-flex px-2.5 py-1 rounded-full bg-amber-100 dark:bg-amber-500/15 text-amber-900 dark:text-amber-200 text-[11px] font-bold">
            ...
            <div>
              <div className="text-4xl font-bold text-emerald-800 dark:text-emerald-300 font-['Outfit']">{precioTexto(d)}</div>
              {totalEstimado != null && <div className="text-xs text-stone-500">Aprox. {dinero.format(totalEstimado)} por el animal</div>}
            ...
            {d.referencia && (
              <div className="rounded-2xl bg-stone-50 dark:bg-white/5 p-3 text-xs space-y-1">
                ...
                {diferenciaReferencia != null && (
                  <div className={diferenciaReferencia > 10 ? "text-rose-700" : diferenciaReferencia < -10 ? "text-emerald-700" : "text-stone-600"}>
          ...
          {d.contraparte && (
            <div className={`${CAJA} p-5 space-y-2 border-emerald-300`}>
              ...
              {d.yaCalifique && <p className="text-xs text-emerald-700">Ya calificaste este trato.</p>}
            </div>
          )}
            ...
            <div className={`${CAJA} p-5 space-y-3`}>
              {aceptadaMia ? (
                aceptadaMia.traspasado ? (
                  <p className="text-sm font-bold text-emerald-800 dark:text-emerald-300">Compraste este animal y ya está en tu hato.</p>
                ) : (
                  <>
                ...
                >
                  ...
                  <div className="relative">
                    ...
                    <input type="number" min="0" step="1" className={`${INPUT} pl-7`} placeholder="Monto total que ofreces" value={monto} onChange={(e) => setMonto(e.target.value)} />
                  </div>
        ...
        <div className={`${CAJA} p-6 space-y-4`}>
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-stone-500">Ordenadas de mayor a menor. Pides {precioTexto(d)}.</p>
            </div>
          ...
          {(d.ofertas ?? []).length === 0 ? (
            ...
            <div className="grid md:grid-cols-2 gap-3">
              {(d.ofertas ?? []).map((o, i) => {
                const pct = Math.round((o.monto / d.precio) * 100);
                return (
                  <div key={o.id} className={`rounded-2xl border p-4 space-y-2 ${o.estado === "ACEPTADA" ? "border-emerald-400 bg-emerald-50/50" : i === 0 && o.estado === "PENDIENTE" ? "border-amber-300 bg-amber-50/40" : "border-stone-200 dark:border-white/10"}`}>
                    <div className="flex items-start justify-between gap-2">
                      {o.comprador && <PerfilFinca perfil={o.comprador} rol="COMPRADOR" />}
                      <div className="text-right shrink-0">
                        <div className="text-xl font-bold text-emerald-800 dark:text-emerald-300 font-['Outfit']">{dinero.format(o.monto)}</div>
                        <div className="text-[11px] text-stone-500">{pct}% de lo que pides</div>
                      </div>
                    </div>
                    <div className="h-1.5 rounded-full bg-stone-100 overflow-hidden">
                      <div className={`h-full ${pct >= 100 ? "bg-emerald-600" : pct >= 90 ? "bg-amber-500" : "bg-rose-400"}`} style={{ width: `${Math.min(pct, 100)}%` }} />
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 pt-1">
                      <button className="text-xs font-bold text-emerald-800 cursor-pointer" onClick={() => setCompradorChat(o.compradorTenantId)}>Abrir chat</button>
      ...
      <div className="grid lg:grid-cols-5 gap-6">
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className={`${CAJA} p-6`}>
            <h2 className="font-bold text-stone-900 dark:text-white font-['Outfit'] mb-1">Curva de peso</h2>
            <p className="text-xs text-stone-500 mb-4">Pesajes registrados por la finca en Aurora.</p>
            {d.pesos.length === 0 ? (
              ...
              <div className="divide-y divide-stone-100 dark:divide-white/5">
                {d.vacunas.map((v, i) => (
                  <div key={i} className="flex justify-between gap-3 py-2 text-sm">
                    <span className="text-stone-800 dark:text-white">{v.nombre}{v.enfermedadPrevenida ? <span className="text-stone-500"> · {v.enfermedadPrevenida}</span> : null}</span>
            ...
            compradorChat !== undefined ? (
              <Chat
                ...
                cabecera={compradores.size > 1 ? (
                  <select className={`${INPUT} mb-3`} value={compradorChat} onChange={(e) => setCompradorChat(Number(e.target.value))}>
      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((i) => (
          <button key={i} type="button" onClick={() => setEstrellas(i)} className="cursor-pointer" aria-label={`${i} estrellas`}>
            <svg className={`w-7 h-7 ${i <= estrellas ? "text-amber-500" : "text-stone-300"}`} viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
