...
import {
  CATEGORIAS, RAZAS_COMUNES, ESTADOS_VE, CAJA, INPUT, BOTON, numero, precioTexto, edadTexto, sexoTexto,
...
export default function Explorar({ onAbrir }: { onAbrir: (id: number) => void }) {
  ...
  return (
    <div className="space-y-8">
      {/* PORTADA */}
      <section className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-emerald-950 via-emerald-900 to-emerald-800 text-white px-6 py-10 sm:px-10 sm:py-14">
        <div className="absolute -right-20 -top-20 w-80 h-80 rounded-full bg-amber-400/10 blur-3xl" aria-hidden="true" />
        <div className="absolute right-10 bottom-0 w-64 h-64 rounded-full bg-emerald-400/10 blur-3xl" aria-hidden="true" />
        <div className="relative max-w-2xl space-y-5">
          <span className="inline-flex px-3 py-1 rounded-full bg-white/10 text-[11px] font-bold tracking-wider uppercase text-amber-200">
            Solo fincas verificadas de Aurora
          </span>
          <h1 className="text-3xl sm:text-5xl font-bold font-['Outfit'] leading-tight">
            Compra y vende ganado con el historial a la vista
          </h1>
          <p className="text-sm sm:text-base text-emerald-100/80">
            Cada animal trae su curva de peso y sus vacunas desde el registro de la finca. Negocia por chat y recibe el animal en tu hato con un clic.
          </p>
          <form
            onSubmit={(e) => { e.preventDefault(); cambiar({ q: busqueda.trim() || undefined }); }}
            className="flex gap-2 bg-white rounded-2xl p-1.5 shadow-xl max-w-xl"
          >
            <input
              value={busqueda}
            ...
            />
            <button type="submit" className="px-5 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 text-stone-950 text-sm font-bold cursor-pointer">Buscar</button>
          </form>
          <div className="flex flex-wrap gap-6 pt-2 text-sm">
            <div><div className="text-2xl font-bold font-['Outfit']">{resumen ? numero.format(resumen.total) : "-"}</div><div className="text-emerald-200/70 text-xs">animales en venta</div></div>
            <div><div className="text-2xl font-bold font-['Outfit']">{resumen ? numero.format(resumen.tratosCerrados) : "-"}</div><div className="text-emerald-200/70 text-xs">tratos cerrados</div></div>
            <div><div className="text-2xl font-bold font-['Outfit']">{resumen?.estados.length ?? "-"}</div><div className="text-emerald-200/70 text-xs">estados con oferta</div></div>
          </div>
        </div>
      </section>
          ...
          {CATEGORIAS.map((c) => {
            ...
            return (
              <button
                ...
                className={`relative overflow-hidden rounded-2xl p-4 h-32 text-left text-white bg-gradient-to-br ${c.fondo} cursor-pointer transition-all ${activa ? "ring-4 ring-amber-400 scale-[1.02]" : "hover:scale-[1.02]"}`}
              >
                <div className="absolute -right-6 -bottom-6 w-24 h-24 rounded-full bg-white/10" aria-hidden="true" />
                <div className="relative h-full flex flex-col justify-between">
                  <div>
                    <div className="font-bold font-['Outfit'] text-base leading-tight">{c.nombre}</div>
                    <div className="text-[11px] text-white/70 leading-snug mt-1">{c.descripcion}</div>
                  </div>
                  <div className="text-xs font-bold text-white/90">{conteoCategoria(c.id)} en venta</div>
                </div>
              </button>
            );
          })}
            ...
            return (
              <button
                ...
                className={`px-3.5 py-1.5 rounded-full text-xs font-bold border cursor-pointer transition-colors ${
                  activa ? "bg-emerald-800 text-white border-emerald-800" : "bg-white dark:bg-white/5 text-stone-700 dark:text-white/70 border-stone-200 dark:border-white/10 hover:border-emerald-600"
                }`}
              >
                {r}{n > 0 && <span className={`ml-1.5 ${activa ? "text-emerald-200" : "text-stone-400"}`}>{n}</span>}
              </button>
            );
          })}
        </div>
      ...
      <section className={`${CAJA} p-4 space-y-3`}>
        <div className="flex flex-wrap items-end gap-3">
          ...
          <button onClick={() => setMasFiltros(!masFiltros)} className="px-3 py-2.5 text-xs font-bold text-emerald-800 dark:text-emerald-300 cursor-pointer">
  ...
  return (
    <article className={`${CAJA} overflow-hidden group flex flex-col`}>
      <button onClick={onAbrir} className="relative aspect-[4/3] bg-stone-100 dark:bg-white/5 cursor-pointer overflow-hidden">
        ...
        <span className="absolute top-3 left-3 px-2.5 py-1 rounded-full bg-stone-950/70 text-white text-[10px] font-bold backdrop-blur">
        ...
        {p.esMia && p.ofertasPendientes > 0 && (
          <span className="absolute top-3 right-3 px-2.5 py-1 rounded-full bg-amber-500 text-stone-950 text-[10px] font-bold">
      ...
      <div className="p-4 flex-1 flex flex-col gap-2">
        ...
        <div className="flex items-baseline justify-between">
          <span className="text-xl font-bold text-emerald-800 dark:text-emerald-300 font-['Outfit']">{precioTexto(p)}</span>
          {p.peso != null && <span className="text-xs font-mono text-stone-600 dark:text-white/60">{numero.format(p.peso)} kg</span>}
        </div>
