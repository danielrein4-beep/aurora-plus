      ...
      {sesiones.length === 0 ? (
        ...
        <div className="space-y-4">
          {sesiones.map((s) => (
            <div
              key={s.id}
              className="p-6 rounded-3xl bg-slate-900/90 border border-white/10 space-y-4 shadow-xl text-xs"
            >
              {/* Encabezado de Sesion */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/10 pb-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-2xl bg-emerald-500/20 text-emerald-400 font-mono font-black flex items-center justify-center border border-emerald-500/30 text-sm">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-slate-400">
                      Fecha: <span className="text-emerald-400 font-bold">{s.fecha_sesion}</span> &bull; Dr(a): {s.odontologo}
                    </div>
                  </div>
                </div>

                <div className="flex items-center gap-2">
                  <span className="px-2.5 py-1 rounded-full bg-white/5 border border-white/10 text-slate-300 text-[10px] font-bold">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                {s.anestesia_administrada && (
                  <div className="p-3 rounded-xl bg-white/5 border border-white/5">
                    ...
                    <span className="text-white">{s.anestesia_administrada}</span>
                  </div>
                )}
                ...
                {s.conductometria_notas && (
                  <div className="p-3 rounded-xl bg-white/5 border border-white/5">
                    ...
                    <span className="text-white">{s.conductometria_notas}</span>
                  </div>
                )}
              </div>
      ...
      {modalNueva && (
        <div className="fixed inset-0 z-[2500] flex items-center justify-center p-4 bg-black/75 backdrop-blur-md">
          <div className="bg-slate-900 border border-white/20 rounded-3xl p-6 max-w-lg w-full text-left space-y-4 shadow-2xl">
            <h4 className="font-['Outfit'] font-black text-xl text-white">Registrar Evolucion en Sillon</h4>
            <form onSubmit={handleGuardarSesion} className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-400 block mb-1">Pieza FDI Tratada</label>
                  <input
                    type="number"
                    min="11"
                    max="85"
                    ...
                    className="w-full p-2.5 rounded-xl bg-white/5 border border-white/15 text-white text-center font-bold font-mono"
                  />
                </div>
                <div>
                  <label className="text-slate-400 block mb-1">Odontologo Responsable *</label>
                  <input
                    type="text"
                    required
                    value={odontologo}
              ...
              <div>
                <label className="text-slate-400 block mb-1">Procedimiento Clinico Realizado *</label>
                <textarea
                  rows={2}
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-400 block mb-1">Tecnica de Aislamiento</label>
                  <select
                    value={aislamiento}
                    onChange={(e) => setAislamiento(e.target.value)}
                    className="w-full p-2.5 rounded-xl bg-slate-800 border border-white/15 text-white font-bold"
                  >
                ...
                <div>
                  <label className="text-slate-400 block mb-1">Anestesia Administrada</label>
                  <input
                    type="text"
                    value={anestesia}
              ...
              <div>
                <label className="text-slate-400 block mb-1">Conductometria / Longitudes de Trabajo / Notas Tecnicas</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-400 block mb-1">Prescripcion Medicamentosa</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-slate-400 block mb-1">Proxima Cita / Conducta</label>
                  <input
                    type="text"
