    ...
    <div className="apple-glass rounded-3xl p-5 sm:p-7 border border-slate-300/60 dark:border-white/10 relative overflow-hidden shadow-xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-200/80 dark:border-white/10">
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto flex-wrap">
          <span className="px-3 py-1.5 text-xs rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-300 font-semibold inline-flex items-center shadow-xs">
          ...
          <span className="px-3 py-1.5 text-xs rounded-full bg-amber-500/10 border border-amber-500/30 text-amber-700 dark:text-amber-300 font-semibold inline-flex items-center shadow-xs">
            <span className="w-2 h-2 rounded-full bg-amber-400 mr-2 shadow-[0_0_6px_rgba(251,191,36,0.8)]" />
            <span>{parcialCount} Parcial</span>
          </span>
          {sinConexionCount > 0 && (
            <span className="px-3 py-1.5 text-xs rounded-full bg-rose-500/10 border border-rose-500/30 text-rose-700 dark:text-rose-300 font-semibold inline-flex items-center shadow-xs">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mt-5">
        {coverageList.length === 0 && (
          <div className="md:col-span-2 lg:col-span-3 xl:col-span-5 rounded-2xl border border-dashed border-slate-300 dark:border-white/10 apple-glass p-8 text-center text-sm text-slate-500 dark:text-white/60">
        ...
        {coverageList.map((item) => {
          ...
          return (
            ...
            >
                ...
                {item.activeSources.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-3">
                    {item.activeSources.map((source, sIdx) => (
                      <span key={sIdx} className="text-[10px] bg-slate-100 dark:bg-white/5 border border-slate-300/60 dark:border-white/10 px-2 py-0.5 rounded-full text-slate-700 dark:text-white/70 font-mono">
                        {source}
                      </span>
                    ))}
                  </div>
              ...
              <div className="text-[11px] text-slate-500 dark:text-white/55 leading-relaxed pt-2.5 border-t border-slate-200/80 dark:border-white/10">
                {item.notes}
              </div>
            </div>
          );
