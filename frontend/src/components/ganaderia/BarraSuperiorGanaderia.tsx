...
export default function BarraSuperiorGanaderia({
  estaOnline, nombreFinca, monedasConfig, pendientesOffline, precioLecheUSD, sincronizandoOffline, tasaBCV,
  tasaCOP, handleSincronizarManual, setModalDatosFiscales, setModalEditarPrecioLeche,
  setModalEditarTasas, setSidebarAbierto,
}: Props) {
  // En celular monedas, leche y datos fiscales se pliegan: primero se ve la finca y la conexión.
  const [verAjustes, setVerAjustes] = useState(false);
  return (
    <>
  <header className="nav-glass border-b border-slate-300/60 dark:border-white/10 px-4 sm:px-6 py-2.5 lg:py-3 flex flex-wrap items-center justify-between lg:justify-end gap-x-3 gap-y-2 sticky top-0 z-30 backdrop-blur-2xl">
    ...
    >
      ...
      <span className="font-semibold text-sm text-slate-900 dark:text-white truncate">{nombreFinca}</span>
    </div>
    ...
    <div className="flex flex-wrap items-center justify-end gap-2 min-w-0">
      ...
      <button
        ...
        className="lg:hidden flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-300/80 bg-white text-[11px] font-semibold text-slate-600 cursor-pointer"
      >
        <IconSettings size={13} />
        <span>{verAjustes ? "Ocultar" : "Ajustes"}</span>
      </button>
    </div>

    <div className={`${verAjustes ? "flex basis-full justify-start" : "hidden"} lg:flex lg:basis-auto flex-wrap items-center gap-2 min-w-0`}>
      {/* Tasas de cambio */}
      <button
        type="button"
        onClick={() => setModalEditarTasas(true)}
        title="Configurar monedas activas y tasas de cambio de la finca"
        className="flex items-center gap-2 apple-glass-pill rounded-full px-3.5 py-1.5 border border-slate-300/80 dark:border-white/15 text-[11px] hover:border-teal-500/50 hover:bg-teal-50 transition-all cursor-pointer group shadow-sm"
      >
      ...
      <button
        ...
        className="flex items-center gap-1.5 apple-glass-pill rounded-full px-3 py-1.5 border border-slate-300/80 dark:border-white/15 text-[11px] hover:border-teal-500/50 hover:bg-teal-50 transition-all cursor-pointer group shadow-sm"
      >
    ...
    <ThemeToggle className="scale-[0.72] origin-right hidden lg:block" />
  </header>
    </>
  );
}
